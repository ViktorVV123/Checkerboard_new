...
const FactoryPage: React.FC = () => {
    ...
    const syncDiffToDB = useCallback(async (prevMap: Map<string, string>, nextMap: Map<string, string>, scenarioId: number, originalData: any[]) => {
        ...
        for (const key of Array.from(allKeys)) {
            ...
            if (nextVal !== undefined) {
                // Правка есть в новом снапшоте — сохраняем
                await saveScenarioEdit(scenarioId, rowId, field, nextVal);
            } else {
                // Правка удалена из нового снапшота — удаляем из БД
                await deleteScenarioEdit(scenarioId, rowId, field);
            }
        }
    }, []);
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            if (activeScenario) {
                // Загружаем правки сценария один раз
                const scenarioData = await getScenarioData(activeScenario.id);
                const editsMap = new Map<number, Record<string, any>>();
                for (const edit of scenarioData) {
                    if (!editsMap.has(edit.id)) editsMap.set(edit.id, {});
                    Object.entries(edit).forEach(([field, value]) => {
                        if (field !== 'id' && value !== null && value !== undefined) {
                            editsMap.get(edit.id)![field] = value;
                        }
                    });
                }

                // Для каждого продукта — загружаем оригинал и применяем правки
                await Promise.all(products.map(async (p) => {
                    const rows = await getProductData(enterprise, p);
                    dataByProduct[p] = rows.map((row) => {
                        const edits = editsMap.get(row.id);
                        if (!edits) return row;
                        const edited = { ...row };
                        Object.entries(edits).forEach(([field, value]) => {
                            edited[field] = Number(value) || value;
                        });
                        return edited;
                    });
                }));
            ...
            const scenarioSuffix = activeScenario ? `_${activeScenario.name}` : '';
            const filename = `${enterprise}${scenarioSuffix}_${new Date().toLocaleDateString('ru-RU').replace(/\./g, '-')}.xlsx`;
    ...
    const handleCellEdit = async (rowId: number, field: string, value: string) => {
        ...
        if (newVal === origVal || (newVal === '0' && origVal === null) || (newVal === null && origVal === null)) {
            ...
            // Пересчитываем вычисляемые поля после удаления правки
            ...
            if (shipmentFields.includes(field)) {
                ...
                if (Math.round(total) === Math.round(origShipment)) {
                    newEdited.delete(`${rowId}-shipmentFact`);
                    await deleteScenarioEdit(activeScenario.id, rowId, 'shipmentFact');
                } else {
            ...
            if (['tradeRemains', 'parkVolume'].includes(field)) {
                ...
                if (Math.round(fc) === Math.round(origFc)) {
                    newEdited.delete(`${rowId}-freeCapacity`);
                    await deleteScenarioEdit(activeScenario.id, rowId, 'freeCapacity');
                } else {
    ...
    const handlePasteMultiple = async (edits: { rowId: number; field: string; value: string }[]) => {
        ...
        // Удаляем мусорные правки из БД
        for (const { rowId, field: f } of toDelete) {
            await deleteScenarioEdit(activeScenario.id, rowId, f);
        }
        // Сохраняем новые правки
    ...
    const handleFillDown = async (rowIds: number[], field: string, value: string) => {
        ...
        for (const rowId of rowIds) {
            ...
            if (shipmentFields.includes(field)) {
                ...
                newEdited.forEach((val, k) => { const [rId, f] = parseEditKey(k); if (rId === rowId) cr[f] = Number(val); });
    ...
    return (
        <div className={s.page}>
            ...
            <div className={s.bottomStrip} onClick={() => setShowBottomPanel(!showBottomPanel)}>
                ...
                {updateInfo?.['Обновлено'] && (<span
                    className={s.bottomStripDate}>Данные обновлены {updateInfo['Обновлено'][''] || Object.values(updateInfo['Обновлено'])[0]}</span>)}
            </div>
                        ...
                        <div className={s.panelBlock}>
                            <div className={s.devSection}>
                                ...
                                <div className={s.devRow}>
                                    ...
                                    <span
                                        className={`${s.devDeviation} ${deviationData.ozhidShipment - deviationData.obrShipment >= 0 ? s.devPositive : s.devNegative}`}>{Math.round(deviationData.ozhidShipment - deviationData.obrShipment).toLocaleString('ru-RU')}</span>
                                </div>
                            </div>
                            <div className={s.devSection}>
                                ...
                                <div className={s.devRow}>
                                    ...
                                    <span
                                        className={`${s.devDeviation} ${deviationData.ozhidExpected - deviationData.obrExpected >= 0 ? s.devPositive : s.devNegative}`}>{Math.round(deviationData.ozhidExpected - deviationData.obrExpected).toLocaleString('ru-RU')}</span>
                                </div>
                            </div>
                    ...
                    {updateInfo && (
                        <div className={s.panelBlock}>
                            ...
                            <table className={s.updateTable}>
                                ...
                                {updateInfo['Обновлено'] && (<tr className={s.updateUpdatedRow}>
                                    <td colSpan={3}>Данные
