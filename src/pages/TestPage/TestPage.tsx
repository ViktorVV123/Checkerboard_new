...
import * as s from './TestPage.module.scss';
import {getProductIndicator, IndicatorColor} from '@/utils/calculations';
import TestDataTable, {DeviationData} from '@/pages/TestPage/TestDataTable';
...
import TestTabs from '@/pages/TestPage/TestTabs';
...
const TestPage: React.FC = () => {
    ...
    const detectEditedProducts = async (scenarioId: number, productList: string[], ent: string) => {
        try {
            ...
            await Promise.all(productList.map(async (p) => { const rows = await getProductData(ent, p); if (rows.some((r) => editedIds.has(r.id))) edited.add(p); }));
    ...
    const displayData = data.map((row) => {
        ...
        editedCells.forEach((value, key) => { const [rowId, field] = key.split('-'); if (Number(rowId) === row.id) { editedRow[field] = Number(value) || value; editedFields.push(field); hasEdits = true; } });
        ...
        try {
            ...
            if (scenarioData.length > 0) {
                ...
                if (relevantRows.length > 0) {
                    ...
                    relevantRows.forEach((row) => { Object.entries(row).forEach(([field, value]) => { if (field !== 'id' && value !== null && value !== undefined) { const orig = data.find((r) => r.id === row.id); const origVal = orig ? String(Math.round(Number(orig[field]) || 0)) : '0'; if (String(Math.round(Number(value) || 0)) !== origVal) newEdited.set(`${row.id}-${field}`, String(value)); } }); });
    ...
    const handleCellEdit = async (rowId: number, field: string, value: string) => {
        ...
        if (String(Math.round(Number(value) || 0)) === origVal) { newEdited.delete(`${rowId}-${field}`); setEditedCells(newEdited); return; }
        newEdited.set(`${rowId}-${field}`, value);
        await saveScenarioEdit(activeScenario.id, rowId, field, value);
        const currentRow = {...originalRow}; newEdited.forEach((val, k) => { const [rId, f] = k.split('-'); if (Number(rId) === rowId) currentRow[f] = Number(val); });
        ...
        setEditedCells(newEdited); setEditedProducts((prev) => new Set(prev).add(product));
    };
    ...
    const handleFillDown = async (rowIds: number[], field: string, value: string) => {
        ...
        for (const rowId of rowIds) {
            const originalRow = data.find((r) => r.id === rowId);
            const origVal = originalRow ? String(Math.round(Number(originalRow[field]) || 0)) : '0';
            if (String(Math.round(Number(value) || 0)) === origVal) { newEdited.delete(`${rowId}-${field}`); continue; }
            newEdited.set(`${rowId}-${field}`, value);
            const shipmentFields = ['railwayShipmentFact', 'pipeShipmentFact', 'mnppShipmentFact', 'waterShipmentFact'];
            if (shipmentFields.includes(field)) { const cr = {...originalRow}; newEdited.forEach((val, k) => { const [rId, f] = k.split('-'); if (Number(rId) === rowId) cr[f] = Number(val); }); newEdited.set(`${rowId}-shipmentFact`, String((Number(cr.railwayShipmentFact) || 0) + (Number(cr.pipeShipmentFact) || 0) + (Number(cr.mnppShipmentFact) || 0) + (Number(cr.waterShipmentFact) || 0))); }
            if (['tradeRemains', 'parkVolume'].includes(field)) { const cr = {...originalRow}; newEdited.forEach((val, k) => { const [rId, f] = k.split('-'); if (Number(rId) === rowId) cr[f] = Number(val); }); newEdited.set(`${rowId}-freeCapacity`, String((Number(cr.parkVolume) || 0) - (Number(cr.tradeRemains) || 0))); }
        }
        setEditedCells(newEdited); setEditedProducts((prev) => new Set(prev).add(product));
    ...
    return (
        <div className={s.page}>
            ...
            {products.length > 0 && <TestTabs items={products} active={product} onSelect={setProduct} indicators={productIndicators} editedProducts={activeScenario ? editedProducts : new Set()} />}
                    ...
                    {updateInfo && (
                        <div className={s.panelBlock}>
                            ...
                            <table className={s.updateTable}>
                                ...
                                {updateInfo['Обновлено'] && (
                                    <tr className={s.updateUpdatedRow}><td colSpan={3}>Данные обновлены {updateInfo['Обновлено'][''] || Object.values(updateInfo['Обновлено'])[0]}</td></tr>
                                )}
                                </tbody>
                            </table>
                        </div>
